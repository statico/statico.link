...
import { secureHeaders } from "@statico/zerodep-node-http-server";
...
export function createApp() {
  ...
  // Route: /_refresh
  app.get("/_refresh", async (c) => {
    if (REFRESH_SECRET && c.req.query("secret") !== REFRESH_SECRET) {
      return c.text("Forbidden", 403);
    }
    linksCache = null;
    cacheTime = 0;
    const links = await fetchLinks();
    return c.text(`Refreshed ${links.size} links\n`);
  });
